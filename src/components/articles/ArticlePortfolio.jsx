...
function ArticlePortfolio({ data }) {
  ...
  const _setAnimationStatus = (animationStatus) => {
    ...
    switch (animationStatus) {
      case AnimationStatus.INVISIBLE:
        divs.forEach((div) => div.classList.add(`grid-item-hidden`));
        break;
      case AnimationStatus.VISIBLE:
        divs.forEach((div) => div.classList.remove(`grid-item-hidden`));
        break;
  ...
  return (
    <Article className={`article-portfolio`} title={parsedData.title}>
            ...
            {expandableFilterResult.map((item, key) => (
              ...
              >
                <div
                  ...
                  style={{
                    cursor: data.id === "portfolio" ? "pointer" : "default",
                  }}
                >
                  <ProjectCard
                    title={item.title}
      ...
      {selectedProject && (
        <Modal show onHide={closeModal} size="xl">
