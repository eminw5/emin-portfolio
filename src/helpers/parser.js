...
export const useParser = () => {
  ...
  const parseArticleData = (articleData, supportedFormats) => {
    ...
    if (
      ...
      supportedFormats.includes(articleData.config.format) === false
    ) {
      console.warn(
        `The format ${articleData.config.format} is invalid for the article with ID ${articleData.id}.`
      );
    }
  ...
  const hasAnyItemWithLocaleFieldNamed = (rawItems, fieldName) => {
    return rawItems.some((item) =>
      getTranslation(item["locales"], fieldName, true)
    );
  };
  ...
  const parseArticleItems = (rawItems, options) => {
    ...
    return rawItems.map((item, key) => {
      ...
      parsedItem.dateInterval = dates
        ? utils.formatDateInterval(
            dates?.start,
            dates?.end,
            selectedLanguageId,
            true,
            options.hideDayFromDates
          )
        : null;
      parsedItem.dateStarted = dates?.start;
      parsedItem.dateEnded = dates?.end;

      parsedItem.links = links
        ? links.map((link, key) => {
            return {
              href: link.href,
  ...
  const bindItemsToCategories = (parsedItems, parsedCategories) => {
    parsedItems.forEach((item) => {
      item.category = parsedCategories.find(
